...
//add it here.
static const MimeMap mimeTypes[] = { { "htm", "text/htm" }, { "html",
		"text/html" }, { "css", "text/css" }, { "js", "text/javascript" }, {
		"txt", "text/plain" }, { "jpg", "image/jpeg" },
		{ "jpeg", "image/jpeg" }, { "png", "image/png" }, { NULL, "text/html" }, //default value
		};
...
const char ICACHE_FLASH_ATTR *httpdGetMimetype(char *url) {
	...
	while (mimeTypes[i].ext != NULL && os_strcmp(ext, mimeTypes[i].ext) != 0)
		i++;
...
int ICACHE_FLASH_ATTR httpdFindArg(char *line, char *arg, char *buff,
		int buffLen) {
	char *p, *e;
	if (line == NULL)
		return 0;
...
int ICACHE_FLASH_ATTR httpFindValueFromArg(char *line, char *arg, char *buff,
		int buffLen) {
	if (strstr(line, arg)) {
...
int ICACHE_FLASH_ATTR httpdGetHeader(HttpdConnData *conn, char *header,
		char *ret, int retLen) {
	...
	while (p < (conn->priv->head + conn->priv->headPos)) {
		while (*p <= 32 && *p != 0)
			p++; //skip crap at start
		//See if this is the header
		if (os_strncmp(p, header, strlen(header)) == 0
				&& p[strlen(header)] == ':') {
...
static void ICACHE_FLASH_ATTR xmitSendBuff(HttpdConnData *conn) {
	if (conn->priv->sendBuffLen != 0) {
		espconn_sent(conn->conn, (uint8_t*) conn->priv->sendBuff,
...
static void ICACHE_FLASH_ATTR httpdSentCb(void *arg) {
	...
	if (r == HTTPD_CGI_NOTFOUND || r == HTTPD_CGI_AUTHENTICATED) {
		os_printf(
				"ERROR! CGI fn returns code %d after sending data! Bad CGI!\n",
				r);
		conn->cgi = NULL; //mark for destruction.
	}
	xmitSendBuff(conn);
...
static void ICACHE_FLASH_ATTR httpdRecvCb(void *arg, char *data,
		unsigned short len) {
	int x;
	char *p, *e;
	char sendBuff[MAX_SENDBUFF_LEN];
	...
	for (x = 0; x < len; x++) {
		if (conn->post->len < 0) {
			...
			conn->post->received++;
			if (conn->post->buffLen >= conn->post->buffSize
					|| conn->post->received == conn->post->len) {
...
static void ICACHE_FLASH_ATTR httpdDisconCb(void *arg) {
	...
	for (i = 0; i < MAX_CONN; i++) {
		if (connData[i].conn != NULL) {
			...
			//disconnect cases.
			if (connData[i].conn->state == ESPCONN_NONE
					|| connData[i].conn->state >= ESPCONN_CLOSE) {
				connData[i].conn = NULL;
				if (connData[i].cgi != NULL)
					connData[i].cgi(&connData[i]); //flush cgi data
